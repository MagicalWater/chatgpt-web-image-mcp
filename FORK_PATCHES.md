...
- Local verification after implementation candidate: focused corrective coverage PASS; `npm run check` PASS; full `npm test` PASS with 89 passed, 0 failed, 1 macOS-only skip out of 90 tests; `npm pack --dry-run` PASS. Production acceptance and the original source-image invocation root-cause check remain OPEN.
- Regression coverage required before acceptance:
  ...
  - CDP cleanup disconnects local ownership without closing the operator browser;
